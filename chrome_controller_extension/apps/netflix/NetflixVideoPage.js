...
export class NetflixVideoPage extends VideoPage
{
    constructor()
    {
        super(
    ...
    moveCurrentTime(tabId, value)
    {
        chrome.scripting.executeScript({
            ...
            args: [ value ],
            func: (value) => {
                const netflixPlayer = netflix.appContext.state.playerApp.getAPI().videoPlayer;
                const player = netflixPlayer.getVideoPlayerBySessionId(netflixPlayer.getAllPlayerSessionIds()[0]);

                player.seek(player.getCurrentTime() + value * 10000);
            }
        });
    }
}